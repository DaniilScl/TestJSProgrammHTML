        ...
        if (jsonStorage) {
            ...
            jsonStorage.rooms.forEach((room) => {
                ...
                if (showLabels) {
                    ...
                    if (hightlightId != null && room.id === hightlightId) {
...
document.querySelector(".yaml-data").addEventListener("keyup", event => {
    ...
    const regexMatch = /rooms:([\s\S]*?)(](?=[^\]]*$))/g.exec(value);
    const roomsString = regexMatch[0];
    const floorplanString = value.replace(roomsString, "");
    //var floorplanRooms = /(-([\s\S]*?)((?:[^\n]+\n){5}))/g.exec(floorplanString);
    const floorplanRooms = floorplanString.match(/(-([\s\S]*?)((?:[^\n]+\n){5}))/g);
    const idx = floorplanString.lastIndexOf('-');
    const lastRoom = floorplanString.substring(idx + 1).trim();
    floorplanRooms.push("- " + lastRoom);

    const storageDataConvert = {
        rooms: []
    };
    ...
    floorplanRooms.forEach((room, roomIndex) => {
        ...
        data.forEach((attr, index) => {
            ...
            if (attr.includes("- name:")) {
                ...
                tmproom = storageDataConvert.rooms.find(x => x.id == roomId);
                if (tmproom) {
                    if (!tmproom.zone) {
                        tmproom.zone = {};
                    }
                ...
                tmproom = storageDataConvert.rooms.find(x => x.id == roomId);
                const yValue = data.find(x => x.includes("y1:"));
                if (tmproom) {
                    if (!tmproom.zone) {
                        tmproom.zone = {};
                    }
